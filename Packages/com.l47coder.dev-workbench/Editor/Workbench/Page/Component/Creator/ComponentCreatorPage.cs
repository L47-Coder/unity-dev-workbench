...
{
    [WorkbenchPage("Component", "Creator", order: 1)]
    internal sealed class ComponentCreatorPage : IPage
    {
        ...
        private static void DrawPreviewCard(string title, params ComponentCreatorState.PreviewItem[] items)
        {
            BeginCard();
            DrawHeader(title);
        ...
        private static Color StatusColor(ComponentCreatorState.PreviewStatus s) => s switch
        {
            ComponentCreatorState.PreviewStatus.Create => CreateColor,
            ComponentCreatorState.PreviewStatus.Write => WriteColor,
